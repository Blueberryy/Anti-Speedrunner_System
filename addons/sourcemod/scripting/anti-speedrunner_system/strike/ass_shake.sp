...
Handle g_hShakeTimers[MAXPLAYERS + 1];

public Action cmdASSShake(int client, int args)
{
	...
	{
		ReplyToCommand(client, "%s Usage: ass_shake <optional - #userid|name> <optional - 0: off|1: on> <optional - duration> <optional - 0: once|1: repeat>", ASS_PREFIX01);
		return Plugin_Handled;
	}
	char sTarget[32];
	GetCmdArg(1, sTarget, sizeof(sTarget));
	if (!bSelectTarget(sTarget, client, toggle, timer, 0, duration, 0.0, ""))
	{
		char target_name[32];
		...
		{
			vShakeSpeedrunners(target_list[iPlayer], client, toggle, true, duration, timer);
		}
		ShowActivity2(client, ASS_PREFIX2, "Used \"ass_shake\" on %s.", target_name);
	}
	g_bShakeMenu[client] = true;
	g_bAdminMenu[client] = false;
	return Plugin_Handled;
...
void vShake(int client)
{
	...
	if (hShakeTarget != null)
	{
		...
		bfWrite.WriteFloat(g_flDuration[client]);
		EndMessage();
	}
}

void vShakeSpeedrunners(int target, int client, int toggle, bool log = true, float duration = 0.0, int timer = 0)
{
	duration == 0.0 ? (g_flDuration[target] = 5.0) : (g_flDuration[target] = duration);
	...
	if ((bIsSurvivor(target) && g_cvASSCountBots.BoolValue) || (bIsHumanSurvivor(target) && !g_cvASSCountBots.BoolValue))
	{
		switch (toggle)
		{
			case 0:
			{
				vKillShakeTimer(target);
				if (bIsHumanSurvivor(target) && log)
				{
					bHasTranslationFile() ? PrintHintText(target, "%s %t", ASS_PREFIX, "ShakeNoInform") : PrintHintText(target, "%s You are not shaking anymore!", ASS_PREFIX);
				}
				for (int iPlayer = 1; iPlayer <= MaxClients; iPlayer++)
				{
					if (bIsHumanSurvivor(iPlayer) && log)
					{
						bHasTranslationFile() ? PrintToChat(iPlayer, "%s %t", ASS_PREFIX01, "ShakeNoAnnounce", target) : PrintToChat(iPlayer, "%s %N is not shaking anymore!", ASS_PREFIX01, target);
					}
				}
			}
			case 1:
				...
				else
				{
					...
					else
					{
						if (!g_bShake[target])
						{
							...
							{
								g_hShakeTimers[target] = CreateTimer(g_flDuration[target], tTimerShakeSpeedrunners, target, TIMER_REPEAT);
							}
						}
					}
					if (bIsHumanSurvivor(target) && log)
					{
						bHasTranslationFile() ? PrintHintText(target, "%s %t", ASS_PREFIX, "ShakeInform") : PrintHintText(target, "%s You are shaking!", ASS_PREFIX);
					}
					for (int iPlayer = 1; iPlayer <= MaxClients; iPlayer++)
					{
						if (bIsHumanSurvivor(iPlayer) && log)
						{
							bHasTranslationFile() ? PrintToChat(iPlayer, "%s %t", ASS_PREFIX01, "ShakeAnnounce", target) : PrintToChat(iPlayer, "%s %N is shaking!", ASS_PREFIX01, target);
						}
					}
				}
			}
		...
		{
			LogAction(client, target, "%s \"%L\" used \"ass_shake\" on \"%L\".", ASS_PREFIX, client, target);
		}
	}
}

void vKillShakeTimer(int client)
{
	g_bShake[client] = false;
	if (g_hShakeTimers[client] != null)
	{
		KillTimer(g_hShakeTimers[client]);
		g_hShakeTimers[client] = null;
	}
}
