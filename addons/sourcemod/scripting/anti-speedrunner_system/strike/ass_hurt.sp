...
public Action cmdASSHurt(int client, int args)
{
	...
	else if (timer > 1 || damage < 1 || toggle > 1 || args > 4)
	{
		ReplyToCommand(client, "%s Usage: ass_hurt <optional - #userid|name> <optional - 0: off|1: on> <optional - damage >= 1> <optional - 0: once|1: repeat>", ASS_PREFIX01);
		return Plugin_Handled;
	}
	char sTarget[32];
